...
# 예제5 readlines : 전체를 읽은 후 라인 단위 리스트로 저장
...
with open('./resource/word_list.csv', 'r') as f: # 문제 CSV 파일 로드
	reader = csv.reader(f)
	# Header Skip
	next(reader)
	for c in reader:
...
q = random.choice(words) # 임의의 단어 선택
...
while turns > 0: # 찬스 카운트가 남아 있을 경우
    # 실패 횟수
    failed = 0
